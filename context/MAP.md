...
- `src/nse_xbrl_parser/__init__.py`
  - Public API exports: `parse_xbrl_facts`, `build_xbrl_view`, `render_xbrl_markdown`, `load_xbrl_model`, `clear_view_cache`, and `__version__`.
...
- `build_xbrl_view` in `view.py`
  - Memoizes built views in-process by file path, mtime, size, and output options; access is lock-guarded for concurrent callers, bypassable with `use_cache=False`, and resettable via `clear_view_cache`.
...
- `clear_view_cache` in `view.py`
  - Public reset for the in-process `build_xbrl_view` cache (long-lived processes, tests, refreshed taxonomy data).
- `taxonomy_store.py`
